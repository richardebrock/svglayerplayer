...
namespace svglayerplay
{
    public class svgDrawArc
    {
        ...
        private static double CalculateVectorAngle(double ux, double uy, double vx, double vy)
        {
            ...
            return svgDrawArc.DoublePI - (ta - tb);
        }
        ...
        public void Draw(GraphicsPath path)
        {
            ...
            double sinPhi = Math.Sin(this.Angle * svgDrawArc.RadiansPerDegree);
            double cosPhi = Math.Cos(this.Angle * svgDrawArc.RadiansPerDegree);
            ...
            double theta1 = svgDrawArc.CalculateVectorAngle(1.0, 0.0, (x1dash - cxdash) / rx, (y1dash - cydash) / ry);
            double dtheta = svgDrawArc.CalculateVectorAngle((x1dash - cxdash) / rx, (y1dash - cydash) / ry, (-x1dash - cxdash) / rx, (-y1dash - cydash) / ry);
            ...
            for (int i = 0; i < segments; ++i)
            {
                ...
                path.AddBezier((float)startX, (float)startY, (float)(startX + dx1), (float)(startY + dy1),
